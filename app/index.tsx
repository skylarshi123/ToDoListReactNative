...
export default function Index() {
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      {/* Input section - Continuous bar with plus icon */}
      <View style={[styles.inputContainer, { backgroundColor: colors.card }]}>
        ...
        <Pressable
          ...
          style={({ pressed }) => [
            ...
            newTodoText.trim() === "" && { opacity: 0.5 },
          ]}
        >
          <Ionicons
            name="add-circle"
            size={24}
            color={newTodoText.trim() === "" ? colors.disabled : colors.primary}
          />
        </Pressable>
      </View>

      {/* Todo count text, a little something extra :), but we could delete if we don't want it */}
      <Text style={[styles.todoCount, { color: colors.subText }]}>
        You have {activeTodos.length} tasks (
